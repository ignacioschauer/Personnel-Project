...
$current_date = date('Y-m-d H:i:s', time() + 2);
        ...
        if ($result->num_rows > 0) {
            ...
            ?>
                <tr id="tr_<?php echo $result_item['id']?>">
                    ...
                    <td class="col-jobTitle d-none d-lg-table-cell d-sm-none" id="jobTitle_<?php echo $result_item['id']?>"><?php echo $result_item['jobTitle']?></td>
                    <td class="col-email d-none d-lg-table-cell d-sm-none" id="email_<?php echo $result_item['id']?>"><?php echo $result_item['email']?></td>
        ...
        if ($result->num_rows > 0) {
            ...
            ?>
                <tr id="tr_<?php echo $result_item['id']?>">
                    ...
                    <td  style="display:none" class="col-location_id" id="location_id_<?php echo $result_item['id']?>"><?php echo $count++?></td>
        ...
        if ($result->num_rows > 0) {
            ...
            ?>
                <tr id="tr_<?php echo $result_item['id']?>">
                    ...
                    <td style="display:none" class="col-location-ids" id="location_id_<?php echo $result_item['id']?>"><?php echo $count++;?></td>
...
// add 
    ...
    $query = 'INSERT INTO personnel (id, firstName, lastName, jobTitle, email, departmentId,p_time) VALUES( DEFAULT , "' .  $_POST["firstName"] . '", "' . $_POST["lastName"] . '", "' . $_POST["jobTitle"] . '", "' . $_POST["email"] . '",' . $_POST["departmentId"] . ',"'.$current_date.'")' ;
    $result = $conn->query($query);
}
// add location
else if(isset($_POST["locationName"])){

    $query = 'INSERT INTO location (name,l_time) VALUES("' . $_POST["locationName"] . '","'.$current_date.'")';
    $result = $conn->query($query);
}
// add Department
else if(isset($_POST["departmentName"])){

    $query = 'INSERT INTO department (name,locationID,d_time) VALUES("' . $_POST["departmentName"] . '","' . $_POST["locationid"] . '","'.$current_date.'")';
    $result = $conn->query($query);
}
// update
    ...
    if(isset($_GET["firstName_update"])){
        $query = 'update personnel set firstName="'.$_GET["firstName_update"].'", lastName="'.$_GET["lastName_update"].'", jobTitle="'.$_GET["jobTitle_update"].'", email="'.$_GET["email_update"].'", departmentId="'.$_GET["departmentId_update"].'" ,p_time= "'.$current_date.'", status=1 where id='.$_GET["update_id"];
    }
    else if(isset($_GET["locationName_update"])){
        $query = 'update location set name="'.$_GET["locationName_update"].'" , l_time="'.$current_date.'",  status=1 where id='.$_GET["update_id"];
    }
    else if(isset($_GET["departmentName_update"])){
        $query = 'update department set name="'.$_GET["departmentName_update"].'", locationID="'.$_GET["locationid_update"].'" , d_time="'.$current_date.'",  status= 1 where id='.$_GET["update_id"];
    }
...
// delete
    ...
    if($_GET["delete_table_name"] == 'Employee_table'){
        $query = 'update personnel set p_time= "'.$current_date.'", status = 2 where id='.$_GET["delete_rows"];
    }
    else if($_GET["delete_table_name"] == 'location_table'){
        $query = 'update location set l_time= "'.$current_date.'",  status = 2 where id='.$_GET["delete_rows"];
    }
    else if($_GET["delete_table_name"] == 'Department_table'){
        $query = 'update department set d_time= "'.$current_date.'", status = 2 where id='.$_GET["delete_rows"];
    }
